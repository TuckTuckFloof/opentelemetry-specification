...
/* SpanReference contains the state that must propagate to child Spans and across
 ...
 */
class SpanReference final
{
public:
  /* A temporary constructor for an invalid SpanReference.
   ...
   * a remote parent
   */
  SpanReference(bool sampled_flag, bool has_remote_parent)
      : trace_id_(),
        span_id_(),
  ...
  SpanReference(TraceId trace_id,
                SpanId span_id,
  ...
  SpanReference(const SpanReference &ctx) = default;

  SpanReference &operator=(const SpanReference &ctx) = default;

  bool operator==(const SpanReference &that) const noexcept
  {
  ...
  static SpanReference GetInvalid() { return SpanReference(false, false); }
